...
const HomeServiceSectionManager = () => {
    ...
    const fetchSection = async () => {
        try {
            const response = await api.get('/admin/content/home-service-section');
            ...
            console.error('Error fetching home service section:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                setFormData(prev => ({ ...prev, sideImage: response.data.data.url }));
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            await api.put('/admin/content/home-service-section', formData);
            alert('Our Services section updated successfully!');
        } catch (error) {
            console.error('Error updating home service section:', error);
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-6 border-b border-gray-100 bg-gray-50/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-lg font-bold text-gray-900">Our Services Section</h3>
                        <p className="text-sm text-gray-500">Manage the side image for the core offerings grid</p>
                    </div>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                <div className="space-y-4">
                    <label className="block text-sm font-semibold text-gray-700">Side Image</label>
                    <div className="flex flex-col md:flex-row items-start gap-6">
                        <div className="w-full md:w-1/3 aspect-[3/4] bg-gray-100 rounded-xl border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden relative group">
                            {formData.sideImage ? (
                                <>
                                    <img src={formData.sideImage} alt="Preview" className="w-full h-full object-cover" />
                        ...
                        <div className="flex-1 space-y-4">
                            <div className="bg-amber-50 p-4 rounded-lg border border-amber-100">
                                ...
                                <ul className="text-xs text-amber-700 space-y-1">
                                    <li>• Width: ~800px</li>
                                    <li>• Height: ~1000px</li>
                                    <li>• Aspect Ratio: Vertical (e.g. 3:4)</li>
                                    <li>• Max Size: 20MB</li>
                                </ul>
                            </div>
                            <div className="flex items-center gap-2 px-1">
                                <input
                                    type="checkbox"
                                    id="serviceSectionActive"
                                    checked={formData.isActive}
                                    onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })}
                                    className="rounded border-gray-300 text-[#001B2F] focus:ring-[#001B2F]"
                                />
                                <label htmlFor="serviceSectionActive" className="text-sm font-medium text-gray-700">
                                    Section visibility is managed by core offerings setup
                                </label>
                            </div>
                        </div>
                    </div>
...
export default HomeServiceSectionManager;
